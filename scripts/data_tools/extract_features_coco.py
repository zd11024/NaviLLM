...
def process_features(proc_id, out_queue, scenevp_list, args):
    ...
    for i, batch in enumerate(batched(scenevp_list, args.batch_size)):
        ...
        for i, item in enumerate(batch):
            out_queue.put((item["image_id"], outs[i], []))
...
def main(args):
    ...
    for split in ["train2017", "val2017", "test2017"]:
        for filename in os.listdir(os.path.join(args.image_dir, split)):
            image_list.append({
                "image_id": filename.split(".")[0],
                "path": os.path.join(args.image_dir, split, filename),
            })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_dir', type=str, default="data/images/coco/", help="the path of coco 2017 dir")
    parser.add_argument("--output_file", type=str, default="data/eva_features/coco_EVA02-CLIP-L-14-336.hdf5", help="the path of output features")
